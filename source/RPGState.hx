...
class RPGState extends MusicBeatState
{
  ...
  function RelightenedLevelInit()
	{
      ...
      curtain.Flash(2,3);
      new FlxTimer().start(2,   function(timer:FlxTimer){player.currentState = "movement";}  , 1);
      ...
      //Dialogs
      var dialogTrig = CreateDialogTrigger(["A small pink penguin.", "It seems to be growing at the same rate of the tree."], [FlxColor.WHITE, FlxColor.WHITE], [1, 2]);
      ...
      dialogTrig = CreateDialogTrigger(["On the ground sits a shining star. ", "Even if it's useless for you...", "It fills you with DETERMINATION"], [FlxColor.WHITE, FlxColor.WHITE, FlxColor.YELLOW], [2.5, 2, 2.5], TransitionToXgaster);
      dialogTrig.loadGraphic(Paths.image('ut_save', 'shared'), true, 20, 19);
      dialogTrig.scale.set(1.5,1.5);
      dialogTrig.animation.add('idle', [0, 1], 8, true);
      dialogTrig.animation.play("idle");
  ...
  //Dialog Functions
  function CreateDialogTrigger(texts:Array<String>, ?colors:Array<FlxColor>=null, ?timers:Array<Float>=null, ?optionalComplete:Void->Void, ?optionalSFXArray:Array<String>):DialogTrigger{
  ...
  function CreateDialogSequence(strings:Array<String>){
      ...
      if (strings.length == 0){
          ...
          if (currInteractable.onCompleteCallback != null) currInteractable.onCompleteCallback();
          DeleteDialog(box);
          player.currentState = "movement";
          return;
      }
  ...
  function DeleteDialog(dialog:DialogBox){
      try{
          if (hud.members.contains(dialog) )hud.remove(dialog);
      }
      catch(e) {
  ...
  //Level Functions
  function LoadLevelCollision(levelName:String, tilemapName:String, xoffset:Int = 0, yoffset:Int = 0) : FlxTilemapExt {
  ...
  function TransitionToXgaster() : Void {
     ...
     for (j in 0...2){
       for (i in 0...flamesCount){
           ...
           fire.alpha = 0;
           fire.animationCallbacks["start"] = function() return {fire.animation.play("idle");};
           ...
           if (i == 0 && j == 0){
              ...
              dialogTrig.onTriggerEnterCallback = function() return { fire.animation.play("start"); fire.alpha = 1; FlxG.sound.play(Paths.sound('litfire', 'shared'), 0.6); fireAmbientSound.play(); };
           }
           interactableCollisions.add(dialogTrig);
       }
     }
    ...
    function SpawnCircleAndXGaster() : Void {
        var spawnFlame = function(timer:FlxTimer){
            ...
            if (i == 0){
                ...
                //Fuck haxeFlixel lol
                FlxTween.num(FlxG.camera.scroll.y, FlxG.camera.scroll.y-250, 1.5, {ease: FlxEase.sineInOut}, function(twn:Float){FlxG.camera.scroll.y = twn;});
                FlxTween.tween(xgaster, {alpha: 1}, 1, {startDelay: 6.4, onComplete:
                  function(twn:FlxTween){
                    ...
                    var loadLevel = function(tmr:FlxTimer)
                    {
                      ...
                      var flashBalls = function(tmr:FlxTimer){ balls.alpha = 1-balls.alpha; };
                      var flashFunc =  function(tmr:FlxTimer){ new FlxTimer().start(0.07, flashBalls, 6);  };
                      new FlxTimer().start(0.52, flashFunc);

                      var switchToSong = function(tmr:FlxTimer){ LoadingState.loadAndSwitchState(new PlayState(), true); };
                ...
                fire.alpha = 1;
                fire.animationCallbacks["start"] = function() return { fire.animation.play("idle"); fire.alpha = 1; FlxG.sound.play(Paths.sound('litfire', 'shared'), 0.6); };
                fire.animation.play("start");
                groundLayer.add(fire);
            }
        }
